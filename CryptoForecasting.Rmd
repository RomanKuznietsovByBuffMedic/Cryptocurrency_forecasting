...
Першим підходом до прогнозування цін на криптовалюту використаємо часові ряди — залежність ціни закриття від часу.
...
ТРЕБА, ЩОБ ДО НАЙНОВІШОЇ ДАТИ В ТАБЛИЦЮ ЗАХОДИЛА ІНФОРМАЦІЯ ПО ВІДКРИТТЮ ЦІНИ, А ВСІ ІНШІ --- ПО ЗАКРИТТЮ
...
if(all(nrow(data) == nrow(drop_na(data)),
       all(diff(data$time) == interval),
       nrow(data) == limit)){
